    ...
    public static void inputThenPrintSumAndAverage() {
        ...
        while (true) {
            ...
            if (isAnInt) {
                ...
                sum += number;
                counter++;
                average = (double) sum/ (double) counter; //calculate average here rather than in the method scope because otherwise i get a compile error because division by 0
                average = Math.round(average);

            } else if (!isAnInt && counter == 0) {
                System.out.println("SUM = 0" + " AVG = 0");
                break;
            } else {
                int avgInInt = (int) average;
                System.out.println("SUM = " + sum + " AVG = " + avgInInt);
                break;
            }
